...
router.post("/register", async (req, res) => {
  try {
    ...
    if (passwordConfirm !== password) {
      return res
        .status(400)
        .json({ message: "Password is not confirmed. Please double check!" });
    }
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!isMatch) {
      return res
        .status(401)
        .json({ message: "Password didn't match. Please double check!" });
    }
    ...
    res
      .status(200)
      .json({ message: "Welcome back user", token, user: simplifiedUser });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: error });
  }
